...
def _exec_cmd(commands=None, flags=None, kvflags=None):
    ...
    try:
        result = __salt__["cmd.run_all"](cmd=cmd, output_loglevel="quiet")
...
def status(
    ...
    flags=None,
    kvflags=None,
    sections=["info", "name", "namespace", "version"],
):
    """
    ...
    if namespace:
        if kvflags:
            ...
            kvflags = {"namespace": namespace}
    exec_status = _exec_dict_return(
        commands=["status", release], flags=flags, kvflags=kvflags
    )
    log.debug(f"exec_status is a {type(exec_status)}")
    if isinstance(exec_status, dict):
        our_return = {}
        for field in sections:
            our_return[field] = exec_status[field]
        return our_return
    else:
        return exec_status
...
def upgrade(
    ...
    """
    if values:
        log.debug(f"upgrade values: {values}")
        values_fd, values_path = salt.utils.files.mkstemp(close_fd=False)
        with os.fdopen(values_fd, "w") as fp:
            salt.utils.yaml.safe_dump(values, fp, default_flow_style=False)
        kvflags.update({"values": values_path})
    if version:
    ...
    log.debug(f"helm upgrade {release} {chart} {flags} {kvflags}")
    upgrade_return = _exec_true_return(
